	...
	public Collection<Collection<String>> findById(Object code)
		...
		try {
			...
			String query = "DECLARE @sqlString nvarchar(MAX); "
					...
					+ "SELECT @numResults = COUNT(DISTINCT CODE) FROM @TempProgrammes; "
					+ "SET @neededNumOfResults = @neededNumOfResults - @numResults; "
					+ "IF (@neededNumOfResults > 0) "
					+ "BEGIN "
					+ "INSERT INTO @TempProgrammes (CODE, NAME, EMAIL, IMAGE, DESCRIPTION, DURATION, ENTRYREQUIREMENTS, COUNSELORNAME, COUNSELORPHONE, DISPLAYSTARTDATE, EXPIRYDATE, PROGRAMMESTATUS, COURSEPROVIDER, MAJOR, CATEGORY, LEVEL, CLASSTYPE, CRTON, CRTBY, MODON, MODBY) "
					//+ "-- GET PROGRAMMES THAT MATCHES INTERESTS AND DSD OF A STUDENT "
					+ "SELECT DISTINCT TOP (@neededNumOfResults) p.CODE, p.NAME, p.EMAIL, p.IMAGE, CAST(p.DESCRIPTION AS VARCHAR(4000)) AS DESCRIPTION, p.DURATION, p.ENTRYREQUIREMENTS, p.COUNSELORNAME, p.COUNSELORPHONE, p.DISPLAYSTARTDATE, p.EXPIRYDATE, p.PROGRAMMESTATUS, p.COURSEPROVIDER, p.MAJOR, p.CATEGORY, p.LEVEL, p.CLASSTYPE, p.CRTON, p.CRTBY, p.MODON, p.MODBY FROM [CAMPUS].[STUDENTINTEREST] si JOIN [CAMPUS].[INTEREST] i ON (i.CODE = si.INTEREST AND si.STUDENT = 1) JOIN [CAMPUS].[MAJORINTEREST] mi ON (i.CODE = mi.INTEREST) JOIN [CAMPUS].[MAJOR] m ON (m.CODE = mi.MAJOR) JOIN [CAMPUS].[PROGRAMME] p ON (m.CODE = p.MAJOR) JOIN [CAMPUS].[PROGRAMMETOWN] pt ON (p.CODE = pt.PROGRAMME) JOIN [CAMPUS].[TOWN] t ON (t.CODE = pt.TOWN) JOIN [CAMPUS].[DSD] dsd ON (dsd.CODE = t.DSD AND dsd.CODE = @studentDSDCode) WHERE p.CODE NOT IN (SELECT CODE FROM @TempProgrammes); "
					+ "SELECT @numResults = COUNT(DISTINCT CODE) FROM @TempProgrammes; "
					+ "SET @neededNumOfResults = @neededNumOfResults - @numResults; "
					+ "END "
					+ "IF (@neededNumOfResults > 0) "
					+ "BEGIN "
					+ "INSERT INTO @TempProgrammes (CODE, NAME, EMAIL, IMAGE, DESCRIPTION, DURATION, ENTRYREQUIREMENTS, COUNSELORNAME, COUNSELORPHONE, DISPLAYSTARTDATE, EXPIRYDATE, PROGRAMMESTATUS, COURSEPROVIDER, MAJOR, CATEGORY, LEVEL, CLASSTYPE, CRTON, CRTBY, MODON, MODBY) "
					//+ "-- GET PROGRAMMES THAT MATCHES INTERESTS AND DISTRICT OF A STUDENT "
					...
					+ "BEGIN "
					+ "INSERT INTO @TempProgrammes (CODE, NAME, EMAIL, IMAGE, DESCRIPTION, DURATION, ENTRYREQUIREMENTS, COUNSELORNAME, COUNSELORPHONE, DISPLAYSTARTDATE, EXPIRYDATE, PROGRAMMESTATUS, COURSEPROVIDER, MAJOR, CATEGORY, LEVEL, CLASSTYPE, CRTON, CRTBY, MODON, MODBY) "
					//+ "-- GET PROGRAMMES THAT MATCHES INTERESTS AND PROVINCE OF A STUDENT "
					...
					+ "BEGIN "
					+ "INSERT INTO @TempProgrammes (CODE, NAME, EMAIL, IMAGE, DESCRIPTION, DURATION, ENTRYREQUIREMENTS, COUNSELORNAME, COUNSELORPHONE, DISPLAYSTARTDATE, EXPIRYDATE, PROGRAMMESTATUS, COURSEPROVIDER, MAJOR, CATEGORY, LEVEL, CLASSTYPE, CRTON, CRTBY, MODON, MODBY) "
					//+ "-- GET PROGRAMMES THAT MATCHES INTERESTS AND COUNTRY OF A STUDENT "
					...
					+ "BEGIN "
					+ "INSERT INTO @TempProgrammes (CODE, NAME, EMAIL, IMAGE, DESCRIPTION, DURATION, ENTRYREQUIREMENTS, COUNSELORNAME, COUNSELORPHONE, DISPLAYSTARTDATE, EXPIRYDATE, PROGRAMMESTATUS, COURSEPROVIDER, MAJOR, CATEGORY, LEVEL, CLASSTYPE, CRTON, CRTBY, MODON, MODBY) "
					//+ "-- GET PROGRAMMES THAT MATCHES ONLY INTERESTS OF A STUDENT (DISREGARDING ANY TOWN-RELATED RELATIONSHIPS) "
					...
					+ "BEGIN "
					+ "INSERT INTO @TempProgrammes (CODE, NAME, EMAIL, IMAGE, DESCRIPTION, DURATION, ENTRYREQUIREMENTS, COUNSELORNAME, COUNSELORPHONE, DISPLAYSTARTDATE, EXPIRYDATE, PROGRAMMESTATUS, COURSEPROVIDER, MAJOR, CATEGORY, LEVEL, CLASSTYPE, CRTON, CRTBY, MODON, MODBY) "
					//+ "-- GET PROGRAMMES THAT MATCHES ONLY TOWN OF A STUDENT (DISREGARDING ANY INTEREST-RELATED RELATIONSHIPS) "
					+ "SELECT DISTINCT TOP (@neededNumOfResults) p.CODE, p.NAME, p.EMAIL, p.IMAGE, CAST(p.DESCRIPTION AS VARCHAR(4000)) AS DESCRIPTION, p.DURATION, p.ENTRYREQUIREMENTS, p.COUNSELORNAME, p.COUNSELORPHONE, p.DISPLAYSTARTDATE, p.EXPIRYDATE, p.PROGRAMMESTATUS, p.COURSEPROVIDER, p.MAJOR, p.CATEGORY, p.LEVEL, p.CLASSTYPE, p.CRTON, p.CRTBY, p.MODON, p.MODBY FROM [CAMPUS].[PROGRAMME] p JOIN [CAMPUS].[PROGRAMMETOWN] pt ON (p.CODE = pt.PROGRAMME) JOIN [CAMPUS].[TOWN] t ON (t.CODE = pt.TOWN) JOIN [CAMPUS].[STUDENT] s ON (t.CODE = s.TOWN and s.CODE = 1) WHERE p.CODE NOT IN (SELECT CODE FROM @TempProgrammes); "
					+ "SELECT @numResults = COUNT(DISTINCT CODE) FROM @TempProgrammes; "
					+ "SET @neededNumOfResults = @neededNumOfResults - @numResults; "
					+ "END "
					+ "IF (@neededNumOfResults > 0) "
					...
					+ "SELECT TOP (@neededNumOfResults) p.CODE, p.NAME, p.EMAIL, p.IMAGE, CAST(p.DESCRIPTION AS VARCHAR(4000)) AS DESCRIPTION, p.DURATION, p.ENTRYREQUIREMENTS, p.COUNSELORNAME, p.COUNSELORPHONE, p.DISPLAYSTARTDATE, p.EXPIRYDATE, p.PROGRAMMESTATUS, p.COURSEPROVIDER, p.MAJOR, p.CATEGORY, p.LEVEL, p.CLASSTYPE, p.CRTON, p.CRTBY, p.MODON, p.MODBY FROM [CAMPUS].[PROGRAMME] p WHERE p.CODE NOT IN (SELECT CODE FROM @TempProgrammes) ORDER BY NEWID(); "
					+ "END "
					+ "SELECT DISTINCT CODE, NAME, EMAIL, IMAGE, CAST(DESCRIPTION AS VARCHAR(4000)) AS DESCRIPTION, DURATION, ENTRYREQUIREMENTS, COUNSELORNAME, COUNSELORPHONE, DISPLAYSTARTDATE, EXPIRYDATE, PROGRAMMESTATUS, COURSEPROVIDER, MAJOR, CATEGORY, LEVEL, CLASSTYPE FROM @TempProgrammes;'; "
