	...
	public Collection<Collection<String>> findById(Object code)
		...
		try {
			...
			String query = "DECLARE @sqlString nvarchar(MAX); "
					...
					+ "SELECT DISTINCT TOP (@neededNumOfResults) p.CODE, p.NAME, p.EMAIL, p.IMAGE, CAST(p.DESCRIPTION AS VARCHAR(4000)) AS DESCRIPTION, p.DURATION, p.ENTRYREQUIREMENTS, p.COUNSELORNAME, p.COUNSELORPHONE, p.DISPLAYSTARTDATE, p.EXPIRYDATE, p.PROGRAMMESTATUS, p.COURSEPROVIDER, p.MAJOR, p.CATEGORY, p.LEVEL, p.CLASSTYPE, p.CRTON, p.CRTBY, p.MODON, p.MODBY, s.TOWN, t.DSD, t.DISTRICT, t.PROVINCE, t.COUNTRY, cp.SHORTNAME, cp.NAME FROM [CAMPUS].[STUDENTINTEREST] si JOIN [CAMPUS].[INTEREST] i ON (i.CODE = si.INTEREST AND si.STUDENT = 1) JOIN [CAMPUS].[MAJORINTEREST] mi ON (i.CODE = mi.INTEREST) JOIN [CAMPUS].[MAJOR] m ON (m.CODE = mi.MAJOR) JOIN [CAMPUS].[PROGRAMME] p ON (m.CODE = p.MAJOR) JOIN [CAMPUS].[PROGRAMMETOWN] pt ON (p.CODE = pt.PROGRAMME) JOIN [CAMPUS].[COURSEPROVIDER] cp ON (cp.CODE = p.COURSEPROVIDER) RIGHT JOIN [CAMPUS].[TOWN] t ON (t.CODE = pt.TOWN) JOIN [CAMPUS].[STUDENT] s ON (t.CODE = s.TOWN AND s.CODE = 1); "
					...
					+ "SELECT DISTINCT TOP (@neededNumOfResults) p.CODE, p.NAME, p.EMAIL, p.IMAGE, CAST(p.DESCRIPTION AS VARCHAR(4000)) AS DESCRIPTION, p.DURATION, p.ENTRYREQUIREMENTS, p.COUNSELORNAME, p.COUNSELORPHONE, p.DISPLAYSTARTDATE, p.EXPIRYDATE, p.PROGRAMMESTATUS, p.COURSEPROVIDER, p.MAJOR, p.CATEGORY, p.LEVEL, p.CLASSTYPE, p.CRTON, p.CRTBY, p.MODON, p.MODBY, cp.SHORTNAME, cp.NAME FROM [CAMPUS].[STUDENTINTEREST] si JOIN [CAMPUS].[INTEREST] i ON (i.CODE = si.INTEREST AND si.STUDENT = 1) JOIN [CAMPUS].[MAJORINTEREST] mi ON (i.CODE = mi.INTEREST) JOIN [CAMPUS].[MAJOR] m ON (m.CODE = mi.MAJOR) JOIN [CAMPUS].[PROGRAMME] p ON (m.CODE = p.MAJOR) JOIN [CAMPUS].[COURSEPROVIDER] cp ON (cp.CODE = p.COURSEPROVIDER) JOIN [CAMPUS].[PROGRAMMETOWN] pt ON (p.CODE = pt.PROGRAMME) JOIN [CAMPUS].[TOWN] t ON (t.CODE = pt.TOWN) JOIN [CAMPUS].[DSD] dsd ON (dsd.CODE = t.DSD AND dsd.CODE = @studentDSDCode) WHERE p.CODE NOT IN (SELECT CODE FROM @TempProgrammes); "
					+ "SELECT @numResults = COUNT(DISTINCT CODE) FROM @TempProgrammes; "
					+ "SET @neededNumOfResults = @neededNumOfResults - @numResults; "
					+ "END "
					+ "IF (@neededNumOfResults > 0) "
					...
					+ "END "
					+ "SELECT DISTINCT CODE, NAME, EMAIL, IMAGE, CAST(DESCRIPTION AS VARCHAR(4000)) AS DESCRIPTION, DURATION, ENTRYREQUIREMENTS, COUNSELORNAME, COUNSELORPHONE, DISPLAYSTARTDATE, EXPIRYDATE, PROGRAMMESTATUS, COURSEPROVIDER, MAJOR, CATEGORY, LEVEL, CLASSTYPE, COURSEPROVIDERSHORTNAME, COURSEPROVIDERNAME FROM @TempProgrammes;'; "
