...
public class CmdDeleteHigherEducationData implements ICommand {
	static Logger log = Logger.getLogger(CmdDeleteHigherEducationData.class.getName());
	...
	public IView execute(IDataHelper helper, IView view) throws SQLException, Exception {
		...
		ArrayList<String> eduData = new ArrayList<>();
		...
		try {
			...
			if ((rows != null) && (rows.length > 0)) {
				for (int i = 0; i < rows.length; i++) {
					...
					int rowId = eduDao.delete(data, connection);
					if (rowId > 0) {
						rowCount++;
					}
				}
